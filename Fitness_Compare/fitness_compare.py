...
def read_data(input_file):
    dt_well = pandas.read_excel('SampleData.xlsx', sheet_name='WellData')
    dt_dictionary = pandas.read_excel('SampleData.xlsx', sheet_name='Dictionary')
...
def calculate_ratio(dt_well):
    part_1 = dt_well[dt_well['TargetType'] == 'Ch1Unknown'].rename(columns={'TargetType': 'TargetType_x', 'Concentration': 'Concentration_x'})
    part_2 = dt_well[dt_well['TargetType'] == 'Ch2Unknown'].reset_index().drop(columns=['index', 'Well']).rename(columns={'TargetType': 'TargetType_y', 'Concentration': 'Concentration_y'})
    dt_well_sl = pandas.concat([part_1, part_2], axis=1)
...
def pivot(dt_master):
    dt_output1 = pandas.pivot_table(data=dt_master, values='Ratio', index=['Strain1', 'Strain2', 'Replicate'], columns='Time Point').rename_axis(None, axis=1)
    return dt_output1
...
def strain_avg(dt_output1):
    temp = dt_output1.droplevel(2)
    dt_output2 = temp.iloc[[0]]
    dt_output2[2] = temp[2].mean()
    dt_output2[4] = temp[4].mean()
    dt_output2[6] = temp[6].mean()
    return dt_output2
...
def write_data(output_file, dt_output1, dt_output2):
    ...
    dt_output1.to_excel(writer, sheet_name='All_data', startrow=0, startcol=0)
    dt_output2.to_excel(writer, sheet_name='Average_data', startrow=0, startcol=0)
    writer.close()
